...
var drone = {
  ...
  _sendNavData: function() {
  	config='CONFIG general:navdata_demo TRUE'
  	drone.socket.send(config, 0, config.length, 5554, HOSTNAME,function(){
  	console.log(config);})
  	cmd = 'AT*CTRL=0\r'
  	drone.socket.send(cmd, 0, cmd.length, 5554,HOSTNAME,function(){
  		console.log('NavDAta')
  		//drone._listen
  		console.log(cmd);})
  		
  	drone.socket.send('CTRL=0\r', 0, 6, 5555, HOSTNAME)
  },
  /*
  _listen: function() {
  	console.log(server.listen(HOSTNAME, 5554))
